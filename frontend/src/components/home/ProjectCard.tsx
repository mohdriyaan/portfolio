...
export function ProjectCard({ project, onOpen }: ProjectCardProps) {
  ...
  return (
    <article ref={ref} className={`project-card reveal ${visible ? 'is-visible' : ''}`}>
      <ProjectScreenshot
      ...
      <div className="project-card-body">
        ...
        <div className="badge-row">
          {project.stack.slice(0, 6).map((item) => <Badge key={item}>{item}</Badge>)}
        </div>

        <div className="project-card-actions">
          <button type="button" className="text-link" onClick={() => onOpen(project.slug)}>Read case study <span>↗</span></button>
